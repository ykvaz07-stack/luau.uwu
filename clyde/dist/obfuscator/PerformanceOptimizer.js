    ...
    switch (expr.type) {
        ...
        case "TableConstructor":
            return expr.fields.some(f => {
                if (f.kind === "index" && hasSideEffects(f.index))
                    return true;
...
function referencesName(expr, name) {
    switch (expr.type) {
        ...
        case "TableConstructor":
            return expr.fields.some(f => {
                if (f.kind === "index" && referencesName(f.index, name))
                    return true;
    ...
    switch (stmt.type) {
        case "AssignmentStatement":
            return stmt.vars.some(v => {
                if (v.type === "Identifier")
                    return v.name === name;
                return referencesName(v, name);
            }) || stmt.values.some(v => referencesName(v, name));
        case "CompoundAssignmentStatement":
            return referencesName(stmt.var, name) || referencesName(stmt.value, name);
        ...
        case "IfStatement":
            return referencesName(stmt.condition, name) ||
                ...
                stmt.elseifClauses.some(c => referencesName(c.condition, name) || c.body.some(s => statementReferencesName(s, name))) ||
...
function transformExpression(expr, level) {
    const loc = expr.loc;
    switch (expr.type) {
        case "BinaryExpression": {
            ...
            if (level >= 2) {
                if (expr.operator === "^" && right.type === "NumberLiteral") {
                    ...
                    if (rv === 3) {
                        const inner = { type: "BinaryExpression", operator: "*", left, right: left, loc };
        ...
        case "TableConstructor": {
            ...
            const fields = expr.fields.map(f => {
                if (f.kind === "index") {
                    ...
                    if (newIndex !== f.index || newValue !== f.value)
                        changed = true;
...
function walkStatement(stmt, level) {
    switch (stmt.type) {
        case "AssignmentStatement": {
        ...
        case "FunctionCallStatement": {
            const call = transformExpression(stmt.call, level);
            return { ...stmt, call };
        }
        case "DoStatement": {
...
function walkStatements(body, level) {
    return body.map(stmt => walkStatement(stmt, level));
}
function eliminateDeadStoresInBody(body) {
    const result = [];
    for (let i = 0; i < body.length; i++) {
        ...
        if (stmt.type === "LocalStatement") {
            ...
            const isReferenced = varNames.some(name => body.slice(i + 1).some(s => statementReferencesName(s, name)));
...
function applyGCOptimizationsInBody(body) {
    const result = [];
    let i = 0;
    while (i < body.length) {
        const stmt = body[i];
...
export function optimizePerformance(ast, options) {
