...
function makeDecodeCall(bytes, key, loc, decoderName) {
...
function makeDecoderStatements(key, loc, decoderName) {
...
function transformExpression(exp, key, decoderName) {
    if (exp.type === "StringLiteral") {
        if (exp.value === "")
            return exp;
        const bytes = encodeString(exp.value, key);
        return makeDecodeCall(bytes, key, exp.loc, decoderName);
    }
    ...
    if (exp.type === "StringInterpolation") {
        return {
            ...exp,
            parts: exp.parts.map((p) => typeof p === "string" ? p : transformExpression(p, key, decoderName)),
        };
    }
    return exp;
}
function transformStatement(stmt, key, decoderName) {
    switch (stmt.type) {
        case "LocalStatement":
            return {
                ...stmt,
        ...
        case "FunctionCallStatement":
            return { ...stmt, call: transformExpression(stmt.call, key, decoderName) };
