...
function countStatements(body) {
    ...
    for (const stmt of body) {
        ...
        if ("thenBody" in stmt) {
            count += countStatements(stmt.thenBody);
        }
        if ("elseBody" in stmt && stmt.elseBody) {
            count += countStatements(stmt.elseBody);
        }
...
function countFunctions(body) {
    ...
    for (const stmt of body) {
        ...
        if ("body" in stmt && Array.isArray(stmt.body)) {
            count += countFunctions(stmt.body);
        }
        if ("thenBody" in stmt) {
            count += countFunctions(stmt.thenBody);
        }
        if ("elseBody" in stmt && stmt.elseBody) {
            count += countFunctions(stmt.elseBody);
        }
        if ("elseifClauses" in stmt) {
            for (const clause of stmt.elseifClauses) {
...
function countLocals(body) {
    ...
    for (const stmt of body) {
        ...
        if ("thenBody" in stmt) {
            count += countLocals(stmt.thenBody);
        }
        if ("elseBody" in stmt && stmt.elseBody) {
            count += countLocals(stmt.elseBody);
        }
        if ("elseifClauses" in stmt) {
            for (const clause of stmt.elseifClauses) {
...
function collectGlobals(body) {
    ...
    function walkBody(stmts, localScope) {
        for (const stmt of stmts) {
            if (stmt.type === "LocalStatement") {
                for (const v of stmt.vars)
                    localScope.add(v.name);
            }
        ...
        for (const stmt of stmts) {
            if (stmt.type === "LocalStatement") {
                ...
                if (stmt.step)
                    walkExpression(stmt.step, localScope);
                walkBody(stmt.body, innerScope);
            }
            else if (stmt.type === "ForInStatement") {
                const innerScope = new Set(localScope);
                for (const v of stmt.vars)
                    innerScope.add(v.name);
                for (const e of stmt.iter)
                    walkExpression(e, localScope);
                walkBody(stmt.body, innerScope);
            }
                ...
                for (const v of stmt.vars) {
                    ...
                    walkExpression(v, localScope);
                }
                for (const val of stmt.values)
                    walkExpression(val, localScope);
            }
            else if (stmt.type === "FunctionCallStatement") {
                walkExpression(stmt.call, localScope);
            }
                ...
                if (stmt.elseBody)
                    walkBody(stmt.elseBody, new Set(localScope));
            }
            else if (stmt.type === "DoStatement") {
                walkBody(stmt.body, new Set(localScope));
            }
            else if (stmt.type === "CompoundAssignmentStatement") {
                walkExpression(stmt.var, localScope);
                walkExpression(stmt.value, localScope);
            }
        }
    }
    function walkExpression(exp, localScope) {
        if (!exp)
            return;
        switch (exp.type) {
            case "Identifier":
                if (!localScope.has(exp.name))
                    globals.add(exp.name);
                break;
...
function detectFeatures(body) {
    ...
    function walkExpressions(node) {
        if (!node || typeof node !== "object")
            return;
        if (node.type === "StringInterpolation")
            features.add("string-interpolation");
        if (node.type === "IfElseExpression")
            features.add("if-else-expression");
        if (node.type === "TypeAssertion")
            features.add("type-assertion");
        if (node.type === "FunctionExpression")
            features.add("anonymous-functions");
        if (node.type === "VarargExpression")
            features.add("varargs");
        if (node.type === "MethodCallExpression")
            features.add("method-calls");
...
//# sourceMappingURL=LuauCompiler.js.map