...
function compileString(luaSource) {
...
function mapBytecode(code, opcodeEncode) {
    const result = [...code];
    let i = 0;
...
function canFuse(start, len, jumpTargets) {
...
function injectCamouflage(code) {
    const boundaries = [];
    let i = 0;
    while (i < code.length) {
        boundaries.push(i);
    ...
    for (let bi = 1; bi < boundaries.length - 1; bi++) {
        if (rng() < 0.12) {
            ...
            const bytes = [64 + camoType];
            for (let a = 0; a < camoType; a++)
...
function injectCamouflageChunk(chunk) {
    chunk.code = injectCamouflage(chunk.code);
    if (chunk.protos) {
        for (const p of chunk.protos)
            injectCamouflageChunk(p);
    }
}
function instrSize(op) {
...
function flattenBytecodeBlocks(code) {
    if (code.length < 20)
        return code;
    ...
    while (i < code.length) {
        ...
        if (jmp) {
            ...
            if (target >= 0 && target < code.length)
                blockBoundaries.add(target);
            if (nextI < code.length)
                blockBoundaries.add(nextI);
        }
        if (op === 31 || op === 41) {
            if (nextI < code.length)
                blockBoundaries.add(nextI);
    ...
    for (let bi = 0; bi < sortedBounds.length; bi++) {
        ...
        if (bStart >= code.length || bEnd <= bStart)
            continue;
    ...
    const oldStartToNewStart = new Map();
    let pos = 0;
...
function computeCtxBit(ctxInit, ctxPrime, pos) {
    const luaIp = pos + 1;
    return (((ctxInit ^ Math.imul(luaIp, ctxPrime)) >>> 0) >>> 16) & 1;
}
function contextOpcodeTransform(code, ctxInit, ctxPrime) {
    const result = [...code];
    let i = 0;
    while (i < result.length) {
        const op = result[i];
...
function contextTransformChunk(chunk, ctxInit, ctxPrime) {
    chunk.code = contextOpcodeTransform(chunk.code, ctxInit, ctxPrime);
    if (chunk.protos) {
        for (const p of chunk.protos)
            contextTransformChunk(p, ctxInit, ctxPrime);
    }
}
function encodeJumpTargets(code, jumpKey) {
    const result = [...code];
    let i = 0;
    while (i < result.length) {
        const op = result[i];
...
function encodeJumpTargetsChunk(chunk, jumpKey) {
    chunk.code = encodeJumpTargets(chunk.code, jumpKey);
    if (chunk.protos) {
        for (const p of chunk.protos)
            encodeJumpTargetsChunk(p, jumpKey);
    }
}
let _nameCounter = 0;
function resetNames() {
...
function encryptByte(b, dk, strategy) {
    if (strategy === 0)
        return b ^ dk;
    if (strategy === 1)
        return (b + dk) & 0xFF;
    if (strategy === 2)
        return b ^ (((dk << 3) | (dk >>> 5)) & 0xFF);
    return (b + (dk ^ 0xAA)) & 0xFF;
}
function encryptStringLazy(bytes, idx1, baseKey, keyPrime, doMutation = false) {
...
function encryptStringFragment(bytes, idx1, baseKey, keyPrime, doMutation = false) {
    const fragSize = doMutation ? (2 + Math.floor(rng() * 5)) : (2 + Math.floor(rng() * 3));
    const nRealFrags = Math.ceil(bytes.length / fragSize);
    const padded = [...bytes];
    while (padded.length < nRealFrags * fragSize)
        padded.push(0);
...
function buildStringPools(K, lazyBaseKey, lazyKeyPrime, doMutation) {
    const nPools = 2 + Math.floor(rng() * 3);
    const pools = [];
    for (let i = 0; i < nPools; i++) {
        const junk = [];
        for (let j = 0; j < 2 + Math.floor(rng() * 5); j++)
            junk.push(Math.floor(rng() * 256));
        pools.push(junk);
    }
    const processedK = [...K];
...
function serializeConstant(v, encodeStrings, xorKey = 0, doFragment = false, xorStep = 0, stringIndex = 0, doConstantFold = false, lazyBaseKey = 0, lazyKeyPrime = 0, doMutation = false) {
    ...
    if (typeof v === "object" && v !== null && typeof v !== "string" && v.__recipe) {
        const r = v;
...
function serializeConstants(K, encodeStrings, xorKey = 0, doFragment = false, xorStep = 0, doConstantFold = false, lazyBaseKey = 0, lazyKeyPrime = 0, doMutation = false) {
...
function wrapCustomCipher(source) {
    ...
    for (let i = 0; i < prefixLen; i++)
        plainBytes.push(Math.floor(rng() * 256));
    for (let i = 0; i < srcBytes.length; i++)
        plainBytes.push(srcBytes[i]);
    const encrypted = [];
    let pk = posSeed;
    ...
    for (let i = 0; i < nFrags; i++)
        frags.push(encoded.slice(i * fragSz, (i + 1) * fragSz));
    const isboxChunks = [];
    const chunkSz = 64;
...
function wrapNestedVM(source) {
    ...
    const dataFrags = [];
    for (let i = 0; i < nDataFrags; i++)
        dataFrags.push(encoded.slice(i * dataFragSz, (i + 1) * dataFragSz));
    ...
    const rInt = () => {
        ...
        if (fmt === 2)
            return `${Math.floor(v / 1000)}_${String(v % 1000).padStart(3, '0')}`;
        return `${v}`;
    };
    const rSpecStr = () => {
        const chars = '!@#$%^&*()_+-=[]{}|;:<>?,./~`';
    ...
    const emitHoneypot = () => {
        ...
        switch (variant) {
            ...
            case 10: {
                ...
                for (let i = 0; i < d; i++)
                    s += `if ${hv1}>${rInt()} then ${hv1}=${hv1}-${rInt()} else `;
                s += `${hv1}=${hv1}+${rInt()} `;
                for (let i = 0; i < d; i++)
                    s += `end `;
                return s;
    ...
    for (const [v, name] of b32methods)
        L.push(`local ${v}=${vB32}[${vSc}(${encSC(name)})]`);
    L.push(emitHoneypot());
    L.push(emitHoneypot());
    const alphaFragVars = [];
    for (let i = 0; i < nAlphaFrags; i++)
        alphaFragVars.push(nn(3));
...
function wrapStubVM(source) {
    ...
    for (let i = 0; i < shuffledAlpha.length; i++) {
        ...
        if (keyDerivVariant === 0)
            stubKey = (stubKey ^ b) & 0xFF;
        else if (keyDerivVariant === 1)
            stubKey = (stubKey + b) & 0xFF;
        else if (keyDerivVariant === 2)
            stubKey = ((stubKey * 31 + b) & 0xFF) | 0;
        else
            stubKey = (i & 1) === 0 ? ((stubKey ^ b) & 0xFF) : ((stubKey + b) & 0xFF);
    }
    stubKey = stubKey || 1;
    ...
    for (let i = 0; i < compressed.length; i++) {
        ...
        if (encVariant === 0)
            encrypted.push((b ^ stubKey) & 0xFF);
        else if (encVariant === 1)
            encrypted.push((b + stubKey) & 0xFF);
        else if (encVariant === 2)
            encrypted.push((256 + b - stubKey) & 0xFF);
        else {
            encrypted.push((b ^ rk) & 0xFF);
            rk = (rk + rollingStep) & 0xFF;
        }
    }
    let encoded = "";
    for (let i = 0; i < encrypted.length; i += 3) {
    ...
    for (const [v, name] of b32Methods)
        L.push(`local ${v}=${b32v}[${a_sc}(${encSC(name)})]`);
    ...
    const dk = (expr) => {
        if (encVariant === 0)
            return `${a_bx}(${expr},${kv})`;
        if (encVariant === 1)
            return `${a_ba}((${expr}-${kv}+256),0xFF)`;
        if (encVariant === 2)
            return `${a_ba}((${expr}+${kv}),0xFF)`;
...
function multiLayerEncrypt(bytes, seed1, seed2, seed3) {
    const data = [...bytes];
...
function packCodeToTable(code, seed1, seed2, seed3) {
    const vle = vleEncode(code);
    const encrypted = multiLayerEncrypt(vle, seed1, seed2, seed3);
    return bytesToLuaString(encrypted);
}
function serializeCode(code, codeXorKey = 0) {
    if (codeXorKey) {
        return `{${code.map(v => v >= 0 ? (v ^ codeXorKey) : v).join(",")}}`;
    }
    return `{${code.join(",")}}`;
}
function serializeProtos(protos, opcodeEncode, encodeStrings, doShuffle, xorKey = 0, doFragment = false, xorStep = 0, codeXorKey = 0, doConstantFold = false, lazyBaseKey = 0, lazyKeyPrime = 0, addFakeProtos = false, protoKeys = { pK: "K", pC: "C", pP: "P", pU: "U", pN: "nParams" }, cipherSeeds, doMutation = false) {
    ...
    if (addFakeProtos) {
        ...
        for (let fi = 0; fi < nFakes; fi++) {
            ...
            for (let ki = 0; ki < fakeKLen; ki++) {
                ...
                if (t === 0)
                    fakeK.push(String(Math.floor(rng() * 1000)));
                else if (t === 1)
                    fakeK.push(rng() > 0.5 ? "true" : "false");
                else
                    fakeK.push(`{${Array.from({ length: 3 + Math.floor(rng() * 5) }, () => Math.floor(rng() * 256)).join(",")}}`);
            }
            let fakeC;
            if (cipherSeeds) {
...
function collatzKeyStream(seed, len) {
    const keys = [];
    let n = seed;
    for (let i = 0; i < len; i++) {
        keys.push(n & 0xFF);
...
function collatzEncodeString(s, seed) {
    const keys = collatzKeyStream(seed, s.length);
    return Array.from(s).map((c, i) => c.charCodeAt(0) ^ keys[i]);
}
function buildEnvSetup(n, level, includeExecutor, targetVersion = "universal") {
    const genv = n.genv;
    const env = n.env;
    if (level === "debug") {
    ...
    for (const a of bootstrapAssigns)
        lines.push(a);
...
function buildHandlerTemplates(n, doNonLinearJumps = false, protoKeys = { pK: "K", pC: "C", pP: "P", pU: "U", pN: "nParams" }) {
    const h = {};
    const { pK, pC, pP, pU, pN } = protoKeys;
    const { K, code, ip, push, pop, top, stack, stackTop, env, protos, upvalues, varargs, varargCount, locals, localBoxes, callBases, callBaseTop, getLocal, setLocal, boxLocal, getMM, arithMM, doReturn, retFromStack, retBase, retTop, retN, retPack, run, resolveK } = n;
    h[0] = `function() end`;
...
function buildHandlerTemplatesSoA(n, doNonLinearJumps = false, protoKeys = { pK: "K", pC: "C", pP: "P", pU: "U", pN: "nParams" }) {
    const h = {};
    const { pK, pC, pP, pU, pN } = protoKeys;
    const { K, code, ip, push, pop, top, stack, stackTop, env, protos, upvalues, varargs, varargCount, locals, localBoxes, callBases, callBaseTop, getLocal, setLocal, boxLocal, getMM, arithMM, doReturn, retFromStack, retBase, retTop, retN, retPack, run, resolveK, soaI, soaA, soaB, soaC } = n;
    h[0] = `function() end`;
    h[1] = `function() ${push}(nil) end`;
    h[2] = `function() ${push}(true) end`;
    h[3] = `function() ${push}(false) end`;
...
function generateFakeHandlers(n, count, usedOps, hxkName = "") {
    const fakes = [];
    let nextFake = 64;
    for (let i = 0; i < count; i++) {
        while (usedOps.has(nextFake))
            nextFake++;
        const op = nextFake++;
        const body = [
...
function generateJunkCode(vmNames) {
    const result = [];
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (vmNames && vmNames.ip && rng() > 0.2) {
            ...
            if (pick === 0)
                result.push(`local ${v}=bit32.bxor(${toHexInt(r1)},${toHexInt(r2)})`);
            else if (pick === 1)
                result.push(`do local ${v}={};${v}[${Math.floor(rng() * 5) + 1}]=bit32.band(${toHexInt(r1)},${toHexInt(r2)});${v}=nil end`);
            else if (pick === 2)
                result.push(`local ${v}=bit32.lrotate(${r1},${Math.floor(rng() * 20) + 1})`);
            else if (pick === 3)
                result.push(`do local ${v}=pcall(function() return ${r1}+${r2} end) end`);
            else if (pick === 4)
                result.push(`local ${v}=select(1,${r1},${r2})`);
            else
                result.push(`local ${v}=type(${r1})=="number" and ${r2} or ${r1}`);
        }
    }
    return result;
}
...
function buildVMFunction(n, opcodeEncode, level, encodeStrings, xorKey = 0, xorStep = 0, codeXorKey = 0, codeHash = 0, integrityRollupHash = 0, lazyBaseKey = 0, lazyKeyPrime = 0, ctxInit = 0, ctxPrime = 0, jumpKeyVal = 0, protoKeys = { pK: "K", pC: "C", pP: "P", pU: "U", pN: "nParams" }, cipherSeeds, doMutation = false, doPooling = false, poolsVarName = "", isTrace = false, traceId = 0, traceCount = 1, traceStart = 0, traceEnd = 0, doGcOpt = false, doPlatformLock = false, doIntegrityRollup = false, doStackCanary = false, soaEnabled = false) {
    ...
    const pushAliases = [n.push];
    const popAliases = [n.pop];
    if (level === "max") {
    ...
    for (let realOp = 0; realOp < 68; realOp++) {
        ...
        if (level === "max" && pushAliases.length > 1) {
            ...
            if (pu !== n.push)
                h = h.split(n.push + '(').join(pu + '(');
            if (po !== n.pop)
                h = h.split(n.pop + '(').join(po + '(');
        }
    ...
    if (level === "max") {
        ...
        for (let g = 0; g < groupCount; g++) {
            ...
            if (start >= shuffledAssignments.length)
                break;
    ...
    if (level === "max") {
        ...
        const aliasDefs = builtinDefs.map(([b, a]) => {
            if (b.includes('.')) {
...
function normalizeVMLevel(level) {
    if (level === "maximum")
        return "max";
    return (level ?? "normal");
}
...
export function generateVM(chunk, options = {}) {
    ...
    const level = normalizeVMLevel(options.level);
    ...
    const cipherSeeds = doMultiLayer ? [
