...
function uniqueRandInt(min, max, exclude, rng) {
    let v;
    do {
...
export function generateBootstrap(config) {
    ...
    const stDecode = uniqueRandInt(100, 9999, usedStates, rng);
    usedStates.add(stDecode);
    const stDecrypt = uniqueRandInt(100, 9999, usedStates, rng);
    usedStates.add(stDecrypt);
    const stVerify = uniqueRandInt(100, 9999, usedStates, rng);
    usedStates.add(stVerify);
    const stLoad = uniqueRandInt(100, 9999, usedStates, rng);
    usedStates.add(stLoad);
    const stExec = uniqueRandInt(100, 9999, usedStates, rng);
    usedStates.add(stExec);
    const stMutate = uniqueRandInt(100, 9999, usedStates, rng);
    usedStates.add(stMutate);
    const junkStates = [];
    for (let i = 0; i < 3; i++) {
    ...
    {
        const splitArray = (data, baseName) => {
    ...
    for (let i = 0; i < junkVarNames.length; i++) {
        ...
        switch (jvType) {
            case 0:
                jvCode = `local ${junkVarNames[i]}=${nBxor}(${1 + Math.floor(rng() * 254)},${nSboxRaw}[${1 + Math.floor(rng() * 255)}])`;
                break;
            case 1:
                jvCode = `local ${junkVarNames[i]}=${nBand}(${nKeyRaw}[${1 + Math.floor(rng() * (xorKey.length - 1))}]+${Math.floor(rng() * 200)},0xFF)`;
                break;
            case 2:
                jvCode = `local ${junkVarNames[i]}=#${nKeyRaw}+${Math.floor(rng() * 100)}`;
                break;
            default:
                jvCode = `local ${junkVarNames[i]}=${nBxor}(${Math.floor(rng() * 256)},${Math.floor(rng() * 256)})`;
                break;
        }
