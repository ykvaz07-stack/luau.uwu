...
import { soaFromFlatCode, soaSerializeXor } from "./soa-encoder.js";
...
function computeTraceSplits(code: number[], numTraces: number): { start: number; end: number }[] {
...
export function generateVM(chunk: BytecodeChunk, options: VMGenOptions = {}): string {
  ...
  if (nesting > 0 && !options.forceSingleVM) {
    ...
    if (nesting >= 2) {
      ...
      try {
        ...
        const middleForced: FeatureFlag[] = [
          "superOperators", "constantFolding", "stringFragment",
  ...
  if (doSplitTraces) {
    ...
    for (const d of codeDecls) parts.push(d);
    ...
    for (const d of dataDecls) parts.push(d);
...
function wrapSoaEncodePost(source: string): string {
...
function wrapAntiBeautifyPost(source: string): string {
  const abTraps = [
