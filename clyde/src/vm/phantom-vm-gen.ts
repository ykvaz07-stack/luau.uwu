...
let _rs = 0;
function sr(s: number): void { _rs = s >>> 0; }
function rn(): number {
...
function rname(short?: boolean): string {
...
function shuf<T>(a: T[]): void {
  for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(rn() * (i + 1)); [a[i], a[j]] = [a[j], a[i]]; }
}

function encKey(n: number): number[] {
  return Array.from({ length: n }, () => Math.floor(rn() * 256));
}

function xorEnc(d: number[], k: number[]): number[] {
...
function esc(s: string): string {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    else if (c < 32 || c > 126) o += `\\x${c.toString(16).padStart(2, '0')}`;
    else o += s[i];
  }
  return o + '"';
}

function ntab(nums: number[]): string {
  if (nums.length === 0) return "{}";
  const parts = nums.map(n => Math.floor(rn() * 3) === 0 ? String(n) : `(${n}+0)`);
  return `{${parts.join(",")}}`;
}

function stable(vals: any[]): string {
...
export function generatePhantomVM(
  chunk: PhantomChunk | { code: number[]; K: any[]; protos?: any[]; maxRegs: number; nParams: number; isVararg: boolean },
  options: PhantomGenOptions = {}
): string {
  ...
  const nEr = rname();
  const nS = rname();
  const nH: string[] = Array.from({ length: PHANTOM_OP_COUNT }, () => rname());
  const nD = rname();
  ...
  for (let i = 0; i < PHANTOM_OP_COUNT; i++) opInv[opShuf[i]] = i;

  const affineA = (() => { let a; do { a = 1 + Math.floor(rn() * 254); } while (gcd(a, 256) !== 1); return a; })();
  const affineB = Math.floor(rn() * 256);
  const affineAInv = modInv(affineA, 256);

  const affEnc = (n: number) => ((affineA * n + affineB) & 0xFF);
  const encCode: number[] = [];
  ...
  out.push(`local ${nBx}=bit32 and bit32.bxor or function(a,b)return a~b end`);
  ...
  for (let h = 0; h < PHANTOM_OP_COUNT; h++) {
    const oi = opShuf[h];
    const body: string[] = [];
    const ra = (r: string) => `${nRegs}[${r}+1]`;
    for (let j = 0; j < Math.floor(rn() * 4); j++) body.push(`local ${rname(true)}=${Math.floor(rn() * 255)}`);
    switch (oi) {
      case 0: body.push(`--`); break;
      case 1: body.push(`${ra("A")}=${nRegs}[1]`); break;
      case 2: body.push(`${ra("A")}=${nL}[${nB}+1]`); break;
      case 3: body.push(`${ra("A")}=nil`); break;
      case 4: body.push(`${ra("A")}=${nB}~=0`); break;
      case 5: body.push(`${ra("A")}=rawget(_G,${nL}[${nB}+1])`); break;
      case 6: body.push(`_G[${nL}[${nB}+1]]=${nRegs}[${nA}+1]`); break;
      case 7: body.push(`${ra("A")}=${nRegs}[${nB}+1][${nC}+1]`); break;
      case 8: body.push(`${nRegs}[${nB}+1][${nC}+1]=${nRegs}[${nA}+1]`); break;
      case 9: body.push(`${ra("A")}={}`); break;
      case 10: body.push(`${ra("A")}=${nRegs}[${nB}+1]+${nRegs}[${nC}+1]`); break;
      case 11: body.push(`${ra("A")}=${nRegs}[${nB}+1]-${nRegs}[${nC}+1]`); break;
      case 12: body.push(`${ra("A")}=${nRegs}[${nB}+1]*${nRegs}[${nC}+1]`); break;
      case 13: body.push(`${ra("A")}=${nRegs}[${nB}+1]/${nRegs}[${nC}+1]`); break;
      case 14: body.push(`${ra("A")}=${nRegs}[${nB}+1]%${nRegs}[${nC}+1]`); break;
      case 15: body.push(`${ra("A")}=${nRegs}[${nB}+1]^${nRegs}[${nC}+1]`); break;
      case 16: body.push(`${ra("A")}=-${nRegs}[${nB}+1]`); break;
      case 17: body.push(`${ra("A")}=not ${nRegs}[${nB}+1]`); break;
      case 18: body.push(`${ra("A")}=#${nRegs}[${nB}+1]`); break;
      case 19: body.push(`local _s=""..${nRegs}[${nB}+1];for _i=2,${nC} do _s=_s..${nRegs}[${nB}+_i] end;${nRegs}[${nA}+1]=_s`); break;
      case 20: body.push(`--`); break;
      case 21: body.push(`${ra("A")}=${nRegs}[${nB}+1]==${nRegs}[${nC}+1]`); break;
      case 22: body.push(`${ra("A")}=${nRegs}[${nB}+1]<${nRegs}[${nC}+1]`); break;
      case 23: body.push(`${ra("A")}=${nRegs}[${nB}+1]<=${nRegs}[${nC}+1]`); break;
      case 24: body.push(`${ra("A")}=${nRegs}[${nB}+1]`); break;
      case 25: body.push(`if ${nRegs}[${nB}+1] then ${nRegs}[${nA}+1]=${nRegs}[${nB}+1] else ${nRegs}[${nA}+1]=${nRegs}[${nC}+1] end`); break;
      case 26: {
      ...
      case 27: body.push(`return ${nRegs}[${nA}+1](table.unpack({${nRegs}[${nA}+2]},1,${nB}))`); break;
      case 28: body.push(`return ${nRegs}[${nA}+1],${nRegs}[${nA}+2],${nRegs}[${nA}+3]`); break;
      case 29: body.push(`if ${nRegs}[${nA}+1]>${nRegs}[${nA}+2] then ${nI}=${nI}+${nC} end`); break;
      case 30: body.push(`${nRegs}[${nA}+1]=${nRegs}[${nA}+1]+${nRegs}[${nA}+3];if ${nRegs}[${nA}+1]<=${nRegs}[${nA}+2] then ${nI}=${nI}+${nC} end`); break;
      case 31: body.push(`--`); break;
      case 32: body.push(`--`); break;
      case 33: body.push(`${ra("A")}=${nFt}[${nB}+1]and function(...)end`); break;
      case 34: body.push(`local _n=select("#",...);for _i=1,_n do ${nRegs}[${nA}+_i]=select(_i,...)end`); break;
      case 35: body.push(`${ra("A")}=${nRegs}[${nB}+1]`); break;
      case 36: body.push(`${nRegs}[${nB}+1]=${nRegs}[${nA}+1]`); break;
      case 37: body.push(`--`); break;
    }
