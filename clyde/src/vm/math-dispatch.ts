...
const SAFE_PRIMES_16_CACHE: number[] = [];

function populateSafePrimes(): void {
  if (SAFE_PRIMES_16_CACHE.length > 0) return;
  ...
  for (let p = 0x101; p < 0xFFFF; p += 2) {
    if (isPrime16(p)) SAFE_PRIMES_16_CACHE.push(p);
    if (SAFE_PRIMES_16_CACHE.length >= 20) break;
  }
}

function isPrime16(n: number): boolean {
  if (n < 2) return false;
  if (n % 2 === 0) return n === 2;
 ...
 */
export function buildMathDispatch(
  opcodeCount: number,
  ...
  // then safe and exact.
  const residues: number[] = new Array(opcodeCount);
...
function gcd32(a: number, b: number): number {
  while (b !== 0) {
    [a, b] = [b, a % b];
  }
  return Math.abs(a);
 ...
 */
export function modInverse32(a: number, p: number): number {
 ...
 */
export function validateMathDispatch(
  cfg: MathDispatchConfig,
  opcodeCount: number,
): string | null {
 ...
 */
export function emitMathDispatch(
  cfg: MathDispatchConfig,
  opVar: string,
  bxorVar: string | null,
  varPrefix: string = "_md",
): { residueVar: string; luaExpr: string } {
  ...
  if (bxorVar) {
    ...
    const luaExpr =
      `((((${cfg.a})*(${bxorVar}))+${cfg.b})%(${cfg.p})+(${cfg.p}))%(${cfg.p})`;
    return { residueVar, luaExpr };
  }
  const luaExpr =
    `(((${cfg.a})*(${opVar})+${cfg.b})%(${cfg.p})+(${cfg.p}))%(${cfg.p})`;
  return { residueVar, luaExpr };
}
...
export function buildDispatchEntries(
  cfg: MathDispatchConfig,
  opcodeCount: number,
): DispatchEntry[] {
