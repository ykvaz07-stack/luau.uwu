...
function isValidKeyByte(b: number): boolean {
 ...
 */
export function computeIntegrityTag(data: number[], key: number[]): number {
  if (key.length === 0) {
 ...
 * frequent = faster).
 */
export function buildIntegrityConfig(
  bytecode: number[],
  rng: () => number,
  opts: {
    keyLen?: number;
 ...
 */
export function emitIntegrityCheck(
  cfg: IntegrityConfig,
  fromPc: number,
  toPc: number,
  byteCodeVar: string,
 ...
 */
export function validateIntegrityConfig(cfg: IntegrityConfig): string | null {
