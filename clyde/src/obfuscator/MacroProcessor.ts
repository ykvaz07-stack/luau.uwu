...
export function processMacros(source: string, options?: MacroProcessorOptions): MacroAnnotations {
  const enabled = options?.enabled !== false;
  const noVirtualize: Set<string> = new Set();
  const encryptFunction: Set<string> = new Set();

  if (!enabled) return { noVirtualize, encryptFunction };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (pendingNoVirtualize || pendingEncryptFunc) {
      ...
      if (name) {
        if (pendingNoVirtualize) noVirtualize.add(name.toLowerCase());
        if (pendingEncryptFunc) encryptFunction.add(name.toLowerCase());
      }
...
export function shouldVirtualize(name: string, annotations: MacroAnnotations): boolean {
  return annotations.noVirtualize.has(name.toLowerCase()) || annotations.noVirtualize.has("*");
}

export function shouldEncrypt(name: string, annotations: MacroAnnotations): boolean {
