...
function assignStmt(vars: { name: string }[], values: Expression[], loc: SourceLocation): Statement {
  return {
...
function localStmt(names: string[], values: Expression[], loc: SourceLocation): Statement {
  return {
    type: "LocalStatement",
...
function binaryExp(left: Expression, operator: string, right: Expression, loc: SourceLocation): Expression {
...
function flattenBlocks(
  ...
  result.push(
    localStmt([stateVar], [binaryExp(numExp(entryState, loc), "~", numExp(xorKey, loc), loc)], loc)
  );
  ...
  const dispatchFields: TableField[] = [];
  ...
  for (let i = 0; i < shuffled.length; i++) {
    ...
    if (hasReturn) {
      const ret = lastStmt as ReturnStatement;
      bodyCopy.pop();
      blockBody = bodyCopy;
      ...
      if (retValues.length > 0) {
            ...
            [
              {
                ...
                fields: retValues.map((v) => ({ kind: "value" as const, value: v })),
                loc,
              },
            ],
            loc
        ...
        blockBody.push(
          assignStmt(
            [{ name: retVar }],
            [{ type: "TableConstructor", fields: [], loc }],
            loc
          )
        );
      }
    ...
    let fnBody: (Statement | LastStatement)[] = [...blockBody];
        ...
        fnBody = [
          {
            ...
            elseBody: [
              assignStmt(
                [{ name: `_j${Math.floor(engine["rng"]() * 100000)}` }],
                [numExp(Math.floor(engine["rng"]() * 1000), loc)],
                loc
              ),
            ],
            loc,
  ...
  result.push(
    localStmt([dispatchVar], [{ type: "TableConstructor", fields: dispatchFields, loc }], loc)
  );

  // While loop
  const tempVar = `_b${stateVar.slice(2)}`;
  const whileCond = binaryExp(
    binaryExp(idExp(stateVar, loc), "~", idExp(xorKeyVar, loc), loc),
    "~=",
    numExp(-1, loc),
    loc
  );
