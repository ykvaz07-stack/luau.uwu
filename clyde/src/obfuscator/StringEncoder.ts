  ...
  if (exp.type === "StringInterpolation") {
    return {
      ...
      parts: exp.parts.map((p) =>
        typeof p === "string" ? p : transformExpression(p, keys, strategies, decoderName, rng, useFragmentation)
      ),
    };
  }
  return exp;
...
function transformStatement(stmt: Statement | LastStatement, keys: number[], strategies: EncryptionStrategy[], decoderName: string, rng: () => number, useFragmentation: boolean): Statement | LastStatement {
  switch (stmt.type) {
    case "LocalStatement":
      return {
        ...stmt,
    ...
    case "FunctionCallStatement":
      return { ...stmt, call: transformExpression(stmt.call, keys, strategies, decoderName, rng, useFragmentation) as CallExpression };
