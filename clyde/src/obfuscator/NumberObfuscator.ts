...
function transformExpression(exp: Expression, engine: MBAEngine, useBitops: boolean): Expression {
  if (exp.type === "NumberLiteral") {
    const n = Number(exp.value);
    if (!Number.isFinite(n)) return exp;
  ...
  if (exp.type === "StringInterpolation") {
    return {
      ...
      parts: exp.parts.map((p) =>
        typeof p === "string" ? p : transformExpression(p, engine, useBitops)
      ),
    };
  }
  return exp;
...
function transformStatement(stmt: Statement | LastStatement, engine: MBAEngine, useBitops: boolean): Statement | LastStatement {
  switch (stmt.type) {
    case "LocalStatement":
      return {
        ...stmt,
    ...
    case "FunctionCallStatement":
      return { ...stmt, call: transformExpression(stmt.call, engine, useBitops) as any };
...
export function obfuscateNumbers(ast: Chunk, options: NumberObfuscatorOptions = {}): Chunk {
